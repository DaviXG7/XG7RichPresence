...
export class WindowConfiguration {
  ...
  checkActivity(data: SetActivity): { error: boolean; message: string } {
    ...
    if (
      data.largeImageText &&
      (data.largeImageText.length < 2 || data.largeImageText.length > 128)
    )
      return { error: true, message: "LargeImageText must be 2-128 chars" };
    if (
      data.smallImageText &&
      (data.smallImageText.length < 2 || data.smallImageText.length > 128)
    )
      return { error: true, message: "SmallImageText must be 2-128 chars" };

    if (
      data.partySize !== undefined &&
      data.partyMax !== undefined &&
      data.partySize > data.partyMax
    )
      return {
        error: true,
        message: "PartySize cannot be greater than PartyMax",
      };

    if (!data.startTimestamp)
      return {
        error: true,
        message: "startTimestamp must be a number or Date",
      };
